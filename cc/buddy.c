...
void printmem(){
	...
	for(i=10; i>=0;i--){
		...
		temp = id[i]->next;
		while(temp!=NULL){
			printf("%d(%s)(%d)->",temp->size,temp->loc==1?"占用":"空闲",temp->start);  //输出内存块的大小、状态、起始位置
			temp=temp->next;
		}
		printf("\n");
	}
...
void init(){	
	...
	note->loc=0;
	note->next=NULL;
	id[10]=(struct block *)malloc(sizeof(struct block));	
	id[10]->next=note;
	id[10]->prior=id[10];
	note->prior=id[10];
	printmem();
...
int merge(int tempId,block *first){
	...
	block * merger=(struct block *)malloc(sizeof(struct block));  //合并后的结点
